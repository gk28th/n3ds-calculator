...
void inputKboard(double long &x, double long &y, bool &inputSucess)
{
	...
	swkbdSetFeatures(&swkbd, SWKBD_FIXED_WIDTH);                // when inputing numbers, they are inputed in boxes
    ...
    if(y == NULL && x == NULL)        // prevent any divide by 0 errors which can cause crashes
        y = 1.0;
